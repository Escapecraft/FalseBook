...
public class cmdLockCart extends Command {

   public cmdLockCart(String pluginName, String syntax, String arguments, String node) {
   ...
   public void execute(String[] args, CommandSender sender) {
      if(!(sender instanceof Player)) {
         ...
         if(!UtilPermissions.playerCanUseCommand(player, "falsebook.cart.lockStorageCart")) {
            ...
            CartHandler.addToLockMode(player);
            ChatUtils.printInfo(player, "[FB-IC]", ChatColor.GRAY, "Rightclick on a storagecart to become an owner of the cart.");
         }
      }
   }
}
