...
public class Break50 implements CartMechanic {
   ...
   public boolean checkRailCreation(BlockPlaceEvent event, Player player) {
      if(!UtilPermissions.playerCanUseCommand(player, "falsebook.cart.brake50x")) {
         BlockUtils.cancelBlockPlace(event, "You are not allowed to build Brake50x-Blocks.");
         return false;
      } else {
         ChatUtils.printInfo(player, "[FB-Cart]", ChatColor.GOLD, "50x BrakeBlock created.");
         return true;
      }
   }
   ...
   public boolean Execute(Minecart cart, Block railBlock, Block block, Block signBlock, CartWorldSettings settings) {
      if(!BlockUtils.isBlockPowered(railBlock) && !BlockUtils.isBlockPowered(block) && !BlockUtils.isBlockPowered(signBlock)) {
         ...
         nowSpeed.setX(nowSpeed.getX() * 0.5D);
         nowSpeed.setZ(nowSpeed.getZ() * 0.5D);
