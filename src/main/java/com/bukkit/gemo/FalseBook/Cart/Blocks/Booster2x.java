...
public class Booster2x implements CartMechanic {
   ...
   public boolean checkRailCreation(BlockPlaceEvent event, Player player) {
      if(!UtilPermissions.playerCanUseCommand(player, "falsebook.cart.booster2x")) {
         BlockUtils.cancelBlockPlace(event, "You are not allowed to build Booster2x-Blocks.");
         return false;
      } else {
         ChatUtils.printInfo(player, "[FB-Cart]", ChatColor.GOLD, "2x BoosterBlock created.");
         return true;
      }
   }
   ...
   public boolean Execute(Minecart cart, Block railBlock, Block block, Block signBlock, CartWorldSettings settings) {
      if(!BlockUtils.isBlockPowered(railBlock) && !BlockUtils.isBlockPowered(block) && !BlockUtils.isBlockPowered(signBlock)) {
         ...
         nowSpeed.setX(nowSpeed.getX() * 2.0D);
         nowSpeed.setZ(nowSpeed.getZ() * 2.0D);
